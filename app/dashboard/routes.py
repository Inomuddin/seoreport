...
# Allowed logo extensions AND their corresponding imghdr type strings.
# We validate both the extension AND the actual file magic bytes.
_ALLOWED_LOGO_EXTENSIONS = {"png", "jpg", "jpeg"}
_IMGHDR_TYPE_MAP = {
    "png": "png",
    "jpg": "jpeg",   # imghdr returns "jpeg" for both .jpg and .jpeg
    "jpeg": "jpeg",
}
...
def overview():
    ...
    all_reports = (
        Report.query
        .join(Client)
        .filter(Client.user_id == current_user.id, Report.status == "complete")
        .all()
    )
    avg_score = (
        round(sum(r.overall_score for r in all_reports if r.overall_score) / len(all_reports))
...
def add_client():
    ...
    if client_limit is not None and current_user.clients.count() >= client_limit:
        flash(
            f"Your {current_user.plan.capitalize()} plan allows up to {client_limit} clients. "
            "Upgrade to add more.",
            "warning",
        )
    ...
    return render_template("dashboard/client_form.html", form=form, title="Add Client")
...
def client_detail(client_id: int):
    ...
    reports = client.reports.order_by(Report.created_at.desc()).all()
    return render_template(
...
@dashboard_bp.route("/clients/<int:client_id>/edit", methods=["GET", "POST"])
@login_required
def edit_client(client_id: int):
    ...
    if form.validate_on_submit():
        ...
        client.contact_email = form.contact_email.data.strip() or None
        client.notes = form.notes.data.strip() or None
        client.auto_report_enabled = form.auto_report_enabled.data
        client.auto_report_day = form.auto_report_day.data or 1
        db.session.commit()
        flash(f"Client \"{client.name}\" updated.", "success")
        return redirect(url_for("dashboard.client_detail", client_id=client.id))

    return render_template(
        "dashboard/client_form.html", form=form, title="Edit Client", client=client
    )
...
@dashboard_bp.route("/clients/<int:client_id>/delete", methods=["POST"])
@login_required
def delete_client(client_id: int):
    ...
    db.session.commit()
    flash(f"Client \"{name}\" and all their reports have been deleted.", "info")
    return redirect(url_for("dashboard.overview"))
...
def profile():
    ...
    if form.validate_on_submit():
        ...
        current_user.agency_name = form.agency_name.data.strip() or None
        current_user.brand_color = form.brand_color.data or "#4F46E5"
        ...
        if form.logo.data:
            ...
            if logo_filename:
                ...
                if current_user.logo_filename:
                    _delete_logo(current_user.logo_filename)
...
def _save_logo(file_storage) -> Optional[str]:
    """
    Save an uploaded logo file and return its filename, or return None on failure.

    Security measures applied:
      1. Reject files exceeding _LOGO_MAX_BYTES (2 MB).
      2. Validate the declared extension against the allowlist.
      3. Read the file magic bytes to confirm the actual image type, not just
         the extension.  This prevents bypasses like renaming a PHP shell to .png.
      4. Always generate a UUID-based filename — never use the original filename,
         even after secure_filename(), to avoid path traversal and enumeration.
    """
    ...
    # ── 1. Read file content (needed for size check and magic bytes) ───────
    content = file_storage.read()
    if len(content) > _LOGO_MAX_BYTES:
        flash("Logo file is too large. Maximum size is 2 MB.", "danger")
        return None
    ...
    # ── 2. Extension check ─────────────────────────────────────────────────
    # Use the original (unsanitised) filename only to extract the extension.
    # Never use it as a path component.
    original_name = file_storage.filename or ""
    if "." not in original_name:
        flash("Logo file must have an extension (png, jpg, jpeg).", "danger")
        return None

    declared_ext = original_name.rsplit(".", 1)[-1].lower()
    if declared_ext not in _ALLOWED_LOGO_EXTENSIONS:
        flash("Only PNG and JPEG images are accepted as logos.", "danger")
        return None

    # ── 3. Magic-byte validation ───────────────────────────────────────────
    # imghdr.what() inspects the first few bytes of the file content,
    # regardless of what the filename claims.
    actual_type = imghdr.what(None, h=content)
    expected_type = _IMGHDR_TYPE_MAP.get(declared_ext)
    if actual_type != expected_type:
        flash("File content does not match the declared image type.", "danger")
        return None

    # ── 4. Generate a safe, random filename — never trust the original ─────
    filename = f"logo_{current_user.id}_{uuid.uuid4().hex}.{declared_ext}"
